...
public:
   ...
   */
  explicit pool_memory_resource(Upstream* upstream_mr, gpuStream_t* gpuStream):
    upstream_mr_{[upstream_mr]() {
      if(upstream_mr == nullptr) { std::logic_error("Unexpected null upstream pointer."); }
      return upstream_mr;
    }()} {
    initialize_pool(gpuStream);
  }
...
protected:
   ...
   */
  block_type try_to_expand(std::size_t try_size, std::size_t min_size, gpuStream_t* stream) {
   ...
   *
   */
  void initialize_pool(gpuStream_t* gpuStream) {
    auto const try_size = [&]() {
    ...
    maximum_pool_size_ = std::nullopt;
   ...
   */
  std::optional<block_type> block_from_upstream(std::size_t size, gpuStream_t* stream) {
    if(size == 0) { return {}; }
