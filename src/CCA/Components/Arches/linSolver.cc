...
//}
  ...
  tsk->requires(Task::WhichDW::NewDW, ALabel, Ghost::AroundCells, 1);   // ghosts needed fro jjacobiblock, not jacobi
  ...
  for (unsigned int i=0;i<d_precMLabel.size();i++){
    //tsk->requires( fineLevel_Q_dw, variable, 0, Task::PatchDomainSpec::FineLevel, 0, Task::MaterialDomainSpec::NormalDomain, d_gn, 0 );
    tskc->requires( Task::WhichDW::NewDW ,d_precMLabel[i], 0, Task::PatchDomainSpec::FineLevel, 0, Task::MaterialDomainSpec::NormalDomain, Ghost::None, 0 );
 ...
 //This allows us to populate ghosts into the "static" a_matrix, and is otherwise unnecessary, this could have been donw ith more complex upstream logic, but this is much more simple.
  ...
  for (unsigned int i=0;i<d_precMLabel.size();i++){
    tsk_hack->requires( Task::WhichDW::NewDW ,d_precMLabel[i], Ghost::AroundCells, cg_ghost+1 ); // +1 not needed for i=0;
                                        ...
                                        const MaterialSet* matls)
{
  ...
  tsk->requires(Task::WhichDW::NewDW, d_cellTypeLabel,       gac, 1);
...
//sched_PreconditionerConstruction( sched,  matls, patches,nullptr);  
  ...
  if(rk_step==0){
    ...
    for (int l = 0; l < maxLevels; ++l) {
      ...
      for (unsigned int i=0;i<d_precMLabel.size();i++){
        task_u->requires(Task::WhichDW::OldDW,d_precMLabel[i],Ghost::AroundCells,cg_ghost+1);
...
////------------------ set up CG solver---------------------//
  ...
  task_i1->requires(Task::WhichDW::NewDW, guess, Ghost::AroundCells, 1);
  task_i1->requires(Task::WhichDW::NewDW, bLabel, Ghost::None, 0);
  task_i1->requires(Task::WhichDW::NewDW, ALabel, Ghost::AroundCells, 1);
...
if (rb_iter == final_iter){
  if (rk_step==0 ){
    task_i2->computesWithScratchGhost
      (d_bigZLabel, nullptr, Uintah::Task::MaterialDomainSpec::NormalDomain,Ghost::AroundCells,cg_ghost);
  ...
  task_i2->requires(Task::WhichDW::NewDW, d_residualLabel, Ghost::AroundCells, cg_ghost); 

    for (unsigned int i=0;i<d_precMLabel.size();i++){
      task_i2->requires(Task::WhichDW::NewDW,d_precMLabel[i],Ghost::AroundCells,cg_ghost+1); //  possibly not needed...
  }
  ...
  task1->requires(Task::WhichDW::NewDW,ALabel,Ghost::None, 0 );
  task1->requires(Task::WhichDW::NewDW,d_smallPLabel, Ghost::AroundCells, 1);
  ...
  task2->requires(Task::WhichDW::NewDW,d_corrSumLabel[cg_iter]);
  task2->requires(Task::WhichDW::NewDW,d_resSumLabel[cg_iter]);
  task2->requires(Task::WhichDW::NewDW,d_littleQLabel, Ghost::None, 0);
  task2->requires(Task::WhichDW::NewDW,d_smallPLabel, Ghost::None, 0);
  ...
  for (int l = maxLevels-2; l > -1; l--) { // Coarsen fine to coarse
    ...
    task_multigrid_up->requires( Task::WhichDW::NewDW ,d_residualLabel, 0, Task::PatchDomainSpec::FineLevel, 0, Task::MaterialDomainSpec::NormalDomain, Ghost::None, 0 );
    if ( cg_iter==0){
  ...
  for (int l = 0; l < maxLevels; ++l) {
    ...
    if (l>0){
        //int offset = 1;
        task_multigrid_down->requires( Task::WhichDW::NewDW ,d_bigZLabel, 0, Task::PatchDomainSpec::CoarseLevel, 0, Task::MaterialDomainSpec::NormalDomain, Ghost::None, 0 );
    }
    ...
    for (int red_black_switch=smoother_iter-1; red_black_switch > -1; red_black_switch--){
      ...
      task_multigrid_smooth->requires(Task::WhichDW::NewDW,d_residualLabel, Ghost::AroundCells, cg_ghost);
      task_multigrid_smooth->requires(Task::WhichDW::NewDW,d_bigZLabel, Ghost::AroundCells,cg_ghost);
      task_multigrid_smooth->modifies(d_bigZLabel);

      for (unsigned int i=0;i<d_precMLabel.size();i++){
        task_multigrid_smooth->requires(Task::WhichDW::NewDW,d_precMLabel[i], Ghost::AroundCells,cg_ghost+1); // NOT REALLY 0 ghost cells, we are taking shortcuts upstream
      }
  ...
  task4->requires(Task::WhichDW::NewDW,d_convMaxLabel[cg_iter]);
  task4->requires(Task::WhichDW::NewDW,d_resSumLabel[cg_iter]);
  task4->requires(Task::WhichDW::NewDW,d_resSumLabel[cg_iter+1]);
  task4->modifies(d_smallPLabel);
  task4->requires(Task::WhichDW::NewDW,d_bigZLabel,Ghost::None, 0);
