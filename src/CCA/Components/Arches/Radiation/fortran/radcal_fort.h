...
extern "C" void radcal_(int* idxlo,
                        int* idxhi,
                        ...
                        int* rgamma_low, int* rgamma_high, double* rgamma_ptr,
...
static void fort_radcal( Uintah::IntVector & idxlo,
                         ...
                         Uintah::OffsetArray1<double> & rgamma,
                         ...
                         double & opl )
{
  Uintah::IntVector abskg_low = abskg.offset();
  Uintah::IntVector abskg_high = abskg.size() + abskg_low - Uintah::IntVector(1, 1, 1);
  ...
  Uintah::IntVector esrcg_low = esrcg.offset();
  Uintah::IntVector esrcg_high = esrcg.size() + esrcg_low - Uintah::IntVector(1, 1, 1);
  ...
  Uintah::IntVector shgamma_low = shgamma.offset();
  Uintah::IntVector shgamma_high = shgamma.size() + shgamma_low - Uintah::IntVector(1, 1, 1);
  ...
  int zz_low = zz.low();
  int zz_high = zz.high();
  Uintah::IntVector pcell_low = pcell.offset();
  Uintah::IntVector pcell_high = pcell.size() + pcell_low - Uintah::IntVector(1, 1, 1);
  ...
  Uintah::IntVector co2_low = co2.offset();
  Uintah::IntVector co2_high = co2.size() + co2_low - Uintah::IntVector(1, 1, 1);
  ...
  Uintah::IntVector h2o_low = h2o.offset();
  Uintah::IntVector h2o_high = h2o.size() + h2o_low - Uintah::IntVector(1, 1, 1);
  ...
  Uintah::IntVector sfv_low = sfv.offset();
  Uintah::IntVector sfv_high = sfv.size() + sfv_low - Uintah::IntVector(1, 1, 1);
  ...
  Uintah::IntVector tg_low = tg.offset();
  Uintah::IntVector tg_high = tg.size() + tg_low - Uintah::IntVector(1, 1, 1);
  ...
  radcal_( idxlo.get_pointer(),
           ...
           &rgamma_low, &rgamma_high, rgamma.get_objs(),
...
C This is the FORTRAN code portion of the file:
      ...
      subroutine radcal(idxlo, idxhi, abskg_low_x, abskg_low_y, 
     ...
     & zz_high, zz, bands, pcell_low_x, pcell_low_y, pcell_low_z, 
     ...
     & tg_low_z, tg_high_x, tg_high_y, tg_high_z, tg, lprobone, 
     & lprobtwo, lplanckmean, lpatchmean, lambda, fraction_low, 
     & fraction_high, fraction, rgamma_low, rgamma_high, rgamma, 
     & sd15_low, sd15_high, sd15, sd_low, sd_high, sd, sd7_low, 
     & sd7_high, sd7, sd3_low, sd3_high, sd3, opl)
     ...
     & tg_low_z:tg_high_z)
      logical*1 lprobone
      logical*1 lprobtwo
      logical*1 lplanckmean
      logical*1 lpatchmean
      integer lambda
