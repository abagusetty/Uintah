...
BadHawkDevol::sched_computeModel( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  ...
  tsk->requires(Task::WhichDW::OldDW, d_weight_label, gn, 0);
  ...
  tsk->requires(Task::WhichDW::OldDW, d_gas_temperature_label, Ghost::AroundCells, 1);
    ...
    if( iMap != LabelToRoleMap.end() ) {
      if( iMap->second == <insert role name here> ) {
        if( eqn_factory.find_scalar_eqn(*iter) ) {
          ...
          tsk->requires(Task::WhichDW::OldDW, d_<insert role name here>_label, Ghost::None, 0);
        } else {
