...
ShaddixHeatTransfer::sched_computeModel( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  ...
  for( CharOxiModelMap::iterator iModel = charoximodels_.begin(); iModel != charoximodels_.end(); ++iModel ) {
    ...
    if( modelNode == d_quadNode) {
      ...
      tsk->requires( Task::WhichDW::OldDW, d_surfacerateLabel, Ghost::None, 0 );
      tsk->requires( Task::WhichDW::OldDW, d_charoxiTempLabel, Ghost::None, 0 );
      tsk->requires( Task::WhichDW::OldDW, d_chargasLabel, Ghost::None, 0 );
    }
  }
  ...
  for( DevolModelMap::iterator iModel = devolmodels_.begin(); iModel != devolmodels_.end(); ++iModel ) {
    ...
    if( modelNode == d_quadNode) {
      ...
      tsk->requires( Task::WhichDW::OldDW, d_devolgasLabel, Ghost::None, 0 );
    }
  }
  ...
  tsk->requires(Task::WhichDW::OldDW, d_weight_label, Ghost::None, 0);
  ...
  tsk->requires(Task::WhichDW::NewDW, iQuad->second, Ghost::None, 0);
  tsk->requires( Task::WhichDW::OldDW, d_fieldLabels->d_CCVelocityLabel, Ghost::None, 0);
  ...
  tsk->requires(Task::WhichDW::OldDW, d_gas_cp_label, Ghost::None, 0);

/* 
  if(_radiation){
    //tsk->requires(Task::WhichDW::OldDW, d_fieldLabels->d_radiationSRCINLabel,  Ghost::None, 0);
    tsk->requires(Task::WhichDW::OldDW, d_abskg_label,  Ghost::None, 0);   
    tsk->requires(Task::WhichDW::OldDW, d_volq_label, Ghost::None, 0);
  }
*/
  ...
  tsk->requires(Task::WhichDW::OldDW, d_gas_temperature_label, Ghost::None, 0);

  const VarLabel* d_specificheat_label = VarLabel::find("specificheat");
  tsk->requires(Task::WhichDW::OldDW, d_specificheat_label, Ghost::None, 0 );
    ...
    if( iMap != LabelToRoleMap.end() ) {
      if ( iMap->second == "particle_temperature") {
        if( dqmom_eqn_factory.find_scalar_eqn(*iter) ) {
          ...
          tsk->requires(Task::WhichDW::OldDW, d_particle_temperature_label, Ghost::None, 0);
        } else {
        ...
        if (dqmom_eqn_factory.find_scalar_eqn(*iter) ) {
          ...
          tsk->requires(Task::WhichDW::OldDW, d_particle_length_label, Ghost::None, 0);
        } else {
          std::string errmsg = "ARCHES: ShaddixHeatTransfer: sched_computeModel(): Invalid variable given in <ICVars> block, for <variable> tag for ShaddixHeatTransfer model.";
        ...
        if (dqmom_eqn_factory.find_scalar_eqn(*iter) ) {
          ...
          tsk->requires(Task::WhichDW::OldDW, d_raw_coal_mass_label, Ghost::None, 0);
        } else {
          std::string errmsg = "ARCHES: ShaddixHeatTransfer: sched_computeModel(): Invalid variable given in <ICVars> block, for <variable> tag for ShaddixHeatTransfer model.";
        ...
        if (dqmom_eqn_factory.find_scalar_eqn(*iter) ) {
          ...
          tsk->requires(Task::WhichDW::OldDW, d_char_mass_label, Ghost::None, 0);
        } else {
          std::string errmsg = "ARCHES: ShaddixHeatTransfer: sched_computeModel(): Invalid variable given in <ICVars> block, for <variable> tag for ShaddixHeatTransfer model.";
          errmsg += "\nCould not find given coal mass  variable \"";
          errmsg += *iter;
