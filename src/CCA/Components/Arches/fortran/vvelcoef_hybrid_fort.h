...
#ifndef fspec_vvelcoef_hybrid
#define fspec_vvelcoef_hybrid

#ifdef __cplusplus

extern "C" void vvelcoef_hybrid_(int* vv_low_x, int* vv_low_y, int* vv_low_z, int* vv_high_x, int* vv_high_y, int* vv_high_z, double* vv_ptr,
                                 ...
                                 int* cvsy_low_x, int* cvsy_low_y, int* cvsy_low_z, int* cvsy_high_x, int* cvsy_high_y, int* cvsy_high_z, double* cvsy_ptr,
                                 double* deltat,
                                 double* grav,
                                 double* dx,
                                 double* dy,
...
static void fort_vvelcoef_hybrid( Uintah::constSFCYVariable<double> & vv,
                                  ...
                                  Uintah::SFCYVariable<double> & cvsy,
                                  double & deltat,
                                  double & grav,
                                  double & dx,
                                  double & dy,
                                  ...
                                  Uintah::IntVector & idxHiV )
{
  ...
  Uintah::IntVector cvsy_low = cvsy.getWindow()->getOffset();
  Uintah::IntVector cvsy_high = cvsy.getWindow()->getData()->size() + cvsy_low - Uintah::IntVector(1, 1, 1);
  int cvsy_low_x = cvsy_low.x();
  int cvsy_high_x = cvsy_high.x();
  int cvsy_low_y = cvsy_low.y();
  int cvsy_high_y = cvsy_high.y();
  int cvsy_low_z = cvsy_low.z();
  int cvsy_high_z = cvsy_high.z();
  vvelcoef_hybrid_( &vv_low_x, &vv_low_y, &vv_low_z, &vv_high_x, &vv_high_y, &vv_high_z, const_cast<double*>(vv.getPointer()),
                    ...
                    &cvsy_low_x, &cvsy_low_y, &cvsy_low_z, &cvsy_high_x, &cvsy_high_y, &cvsy_high_z, cvsy.getPointer(),
                    &deltat,
                    &grav,
                    &dx,
                    &dy,
...
C This is the FORTRAN code portion of the file:
      ...
      subroutine vvelcoef_hybrid(vv_low_x, vv_low_y, vv_low_z, 
     & vv_high_x, vv_high_y, vv_high_z, vv, celltype_low_x, 
     ...
     & uu_low_z, uu_high_x, uu_high_y, uu_high_z, uu, ww_low_x, 
     ...
     & eps_high_y, eps_high_z, eps, cvsy_low_x, cvsy_low_y, cvsy_low_z,
     &  cvsy_high_x, cvsy_high_y, cvsy_high_z, cvsy, deltat, grav, dx, 
     & dy, dz, WALL, INTR, Relimit, idxLoV, idxHiV)
     ...
     & eps_low_z:eps_high_z)
      integer cvsy_low_x, cvsy_low_y, cvsy_low_z, cvsy_high_x, 
     & cvsy_high_y, cvsy_high_z
      double precision cvsy(cvsy_low_x:cvsy_high_x, cvsy_low_y:
     & cvsy_high_y, cvsy_low_z:cvsy_high_z)
