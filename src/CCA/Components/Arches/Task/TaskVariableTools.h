...
/** @brief A container to hold a small amount of other information to pass into
 * the task exe. **/
...
/** @brief A class for managing the retrieval of uintah/so fields during task
 * exe **/
...
public:
  ...
  /** @brief Return a bool to indicate if this Arches Task is a subset of a
   * larger, single Uintah task. **/
  inline bool packed_tasks() { return _tsk_info.packed_tasks; }

  /** @brief return the variable registry **/
  inline std::vector<ArchesFieldContainer::VariableInformation> &
  get_variable_reg() {
    return _var_reg;
  }

  /** @brief Set the references to the variable maps in the Field Collector for
   * easier management of the fields when trying to retrieve from the DW **/
  ...
  typename std::enable_if<
      std::is_base_of<Uintah::constVariableBase<Uintah::GridVariableBase>,
                      T>::value,
      T &>::type
...
// #if defined( _OPENMP ) && defined( KOKKOS_ENABLE_OPENMP )
  ...
  typename std::enable_if<
      std::is_base_of<Uintah::constVariableBase<Uintah::GridVariableBase>,
                      T>::value &&
          std::is_same<MemSpace, Kokkos::HostSpace>::value,
      KokkosView3<PODType, Kokkos::HostSpace>>::type
  get_field(const std::string name, Args... args) {
    return (*(_field_container->get_const_field<T>(name, args...)))
        .getKokkosView();
  }
  ...
  typename std::enable_if<
      std::is_base_of<Uintah::constVariableBase<Uintah::GridVariableBase>,
                      T>::value &&
          std::is_same<MemSpace, Kokkos::CudaSpace>::value,
      KokkosView3<PODType, Kokkos::CudaSpace>>::type
  ...
  typename std::enable_if<
      ...
      KokkosView3<PODType, Kokkos::Experimental::SYCLDeviceUSMSpace>>::type
#endif
  get_field(const std::string name) {
    int which_dw =
        _field_container->get_variable_information(name, true).uintah_task_dw;

    if (which_dw == ArchesFieldContainer::OLDDW ||
        (which_dw == ArchesFieldContainer::LATEST &&
         _tsk_info.time_substep == 0)) {
      return getOldDW()->getGPUDW()->getKokkosView<PODType>(
          name.c_str(), _field_container->getPatch()->getID(),
          _field_container->getMaterialIndex(),
          _field_container->getPatch()->getLevel()->getIndex());
    } else {
      return getNewDW()->getGPUDW()->getKokkosView<PODType>(
          name.c_str(), _field_container->getPatch()->getID(),
          _field_container->getMaterialIndex(),
          _field_container->getPatch()->getLevel()->getIndex());
    }
  ...
  typename std::enable_if<
      ...
      KokkosView3<PODType, Kokkos::Experimental::SYCLDeviceUSMSpace>>::type
#endif
  get_field(const std::string name, ArchesFieldContainer::WHICH_DW which_dw) {
    if (which_dw == ArchesFieldContainer::OLDDW ||
        (which_dw == ArchesFieldContainer::LATEST &&
         _tsk_info.time_substep == 0)) {
      return getOldDW()->getGPUDW()->getKokkosView<PODType>(
...
// #if defined( _OPENMP ) && defined( KOKKOS_ENABLE_OPENMP )
  ...
  inline typename std::enable_if<
      std::is_same<MemSpace, UintahSpaces::HostSpace>::value, T>::type
  get_empty_field() {
    return T();
  }
...
// #if defined( _OPENMP ) && defined( KOKKOS_ENABLE_OPENMP )
  ...
  template <typename T, typename PODType, typename MemSpace>
  inline typename std::enable_if<
      std::is_same<MemSpace, Kokkos::Experimental::SYCLDeviceUSMSpace>::value,
      KokkosView3<PODType, Kokkos::Experimental::SYCLDeviceUSMSpace>>::type
  get_empty_field() {
  ...
  get_unmanaged_uintah_field(FIELD_TYPE &field, const std::string name,
                             const int patch, const int matl_indx,
                             const int DW_index) {
...
// #if defined( _OPENMP ) && defined( KOKKOS_ENABLE_OPENMP )
  ...
  inline typename std::enable_if<
      ...
      get_unmanaged_uintah_field(FIELD_TYPE &field, const std::string name,
                                 const int patch, const int matl_indx,
                                 const int DW_index) {
    field = getNewDW()->getGPUDW()->getKokkosView<ElemType>(
        name.c_str(), patch, matl_indx,
        _field_container->getPatch()->getLevel()->getIndex());
  }
  ...
  inline typename std::enable_if<
      std::is_same<MemSpace, UintahSpaces::HostSpace>::value, void>::type
  get_unmanaged_uintah_field(const std::string name, T &field) {
    _field_container->get_unmanaged_field(name, field);
  }
...
// Legacy tasks need to create a container of things like CCVariable<double>.
// Vector are ok here. Kokkos tasks can't use vectors (not allowed for CUDA).
// Further, the containers must be of Kokkos views of an internal data type. For
// portability, both kinds of types are passed in, and only one of the two is
// used depending on the portability mode.

template <typename T, typename elemType, unsigned int MaxCapacity,
          typename MemSpace>
typename std::enable_if<std::is_same<MemSpace, UintahSpaces::HostSpace>::value,
                        struct1DArray<T, MaxCapacity>>::type
createContainer(int num = MaxCapacity) {
  return struct1DArray<T, MaxCapacity>(
      num); // perform deep copy (should be ok since it is an empty CCVariable?)
}
...
typename std::enable_if<
    std::is_same<MemSpace, Kokkos::HostSpace>::value,
    struct1DArray<KokkosView3<elemType, MemSpace>, MaxCapacity>>::type
createContainer(int num = MaxCapacity) {
...
/** @brief Builds a struct for each variable containing all pertinent uintah DW
 * information **/
void register_variable_work(
    std::string name, ArchesFieldContainer::VAR_DEPEND dep, int nGhost,
    ArchesFieldContainer::WHICH_DW dw,
    ArchesFieldContainer::VariableRegistry &variable_registry,
    const int time_substep, const std::string task_name);
...
void register_variable(
    ...
    std::string task_name =
        "(Arches task name not not communicated to this variable registration)",
...
void register_variable(
    std::string name, ArchesFieldContainer::VAR_DEPEND dep,
    std::vector<ArchesFieldContainer::VariableInformation> &var_reg,
    std::string task_name =
        "(Arches task name not not communicated to this variable registration)",
    const bool temporary_variable = false);
...
/** @brief Inteface to register_variable_work -- this function is overloaded.
 *         This version assumes NewDW and zero ghosts and passes the
 * timesubstep. **/
void register_variable(
    std::string name, ArchesFieldContainer::VAR_DEPEND dep,
    std::vector<ArchesFieldContainer::VariableInformation> &var_reg,
...
template <typename T> struct FieldTool {
  FieldTool(ArchesTaskInfoManager *tsk_info) {
    throw InvalidValue("Error: I should not be in this constructor.", __FILE__,
                       __LINE__);
  }
...
template <> struct FieldTool<CCVariable<double>> {
...
template <> struct FieldTool<constCCVariable<double>> {
  FieldTool(ArchesTaskInfoManager *tsk_info) : m_tsk_info(tsk_info) {}
...
template <> struct FieldTool<SFCXVariable<double>> {
  FieldTool(ArchesTaskInfoManager *tsk_info) : m_tsk_info(tsk_info) {}
...
template <> struct FieldTool<constSFCXVariable<double>> {
  FieldTool(ArchesTaskInfoManager *tsk_info) : m_tsk_info(tsk_info) {}
...
template <> struct FieldTool<SFCYVariable<double>> {
  FieldTool(ArchesTaskInfoManager *tsk_info) : m_tsk_info(tsk_info) {}
...
template <> struct FieldTool<constSFCYVariable<double>> {
  FieldTool(ArchesTaskInfoManager *tsk_info) : m_tsk_info(tsk_info) {}
...
template <> struct FieldTool<SFCZVariable<double>> {
  FieldTool(ArchesTaskInfoManager *tsk_info) : m_tsk_info(tsk_info) {}
...
template <> struct FieldTool<constSFCZVariable<double>> {
  FieldTool(ArchesTaskInfoManager *tsk_info) : m_tsk_info(tsk_info) {}
