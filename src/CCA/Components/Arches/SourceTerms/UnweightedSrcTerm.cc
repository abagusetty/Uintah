...
UnweightedSrcTerm::sched_computeSource( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  ...
  tsk->requires(Task::WhichDW::OldDW, d_areaFractionLabel, Ghost::AroundCells, 1);
  ...
  for (vector<std::string>::iterator iter = _required_labels.begin(); 
    ...
    tsk->requires( Task::WhichDW::OldDW, unwaLabel, Ghost::None, 0 );
    ...
    tsk->requires( Task::WhichDW::NewDW, partVelLabel, Ghost::AroundCells, 1 );
  }
