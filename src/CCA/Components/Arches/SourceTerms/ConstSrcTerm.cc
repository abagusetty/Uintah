...
ConstSrcTerm::ConstSrcTerm( std::string src_name, MaterialManagerP& materialManager,
...
ConstSrcTerm::sched_computeSource( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  ...
  if ( _mult_by_variable ){
    if ( _NewDW_only ){
      tsk->requires( Task::WhichDW::NewDW, VarLabel::find(_mult_var_string), Ghost::None, 0 ); 
    } else {
      if (timeSubStep == 0) { 
        tsk->requires( Task::WhichDW::OldDW, VarLabel::find(_mult_var_string), Ghost::None, 0 ); 
      } else {
        tsk->requires( Task::WhichDW::NewDW, VarLabel::find(_mult_var_string), Ghost::None, 0 ); 
      }
    }
