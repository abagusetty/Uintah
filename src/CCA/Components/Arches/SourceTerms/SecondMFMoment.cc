...
SecondMFMoment::sched_computeSource( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  ...
  if (timeSubStep == 0) {
    tsk->requires( Task::WhichDW::OldDW, densityLabel, Ghost::None, 0 ); 
    tsk->requires( Task::WhichDW::OldDW, scalarDissLabel, Ghost::None, 0 );
  } else {
    tsk->requires( Task::WhichDW::NewDW, densityLabel, Ghost::None, 0 ); 
    tsk->requires( Task::WhichDW::NewDW, scalarDissLabel, Ghost::None, 0 );
  }
