...
CoalGasMomentum::sched_computeSource( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  ...
  for (int iqn = 0; iqn < dqmomFactory.get_quad_nodes(); iqn++){
    ...
    tsk->requires( Task::WhichDW::NewDW, tempgasLabel_x, Ghost::None, 0 );
    ...
    tsk->requires( Task::WhichDW::NewDW, tempgasLabel_y, Ghost::None, 0 );
    ...
    tsk->requires( Task::WhichDW::NewDW, tempgasLabel_z, Ghost::None, 0 );
