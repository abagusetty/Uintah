...
CoalGasHeat::sched_computeSource( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  ...
  for (int iqn = 0; iqn < dqmomFactory.get_quad_nodes(); iqn++){
    ...
    tsk->requires( Task::WhichDW::NewDW, tempgasLabel_m, Ghost::None, 0 );
    ...
    if (m_dest_flag){
      ...
      tsk->requires( Task::WhichDW::NewDW, enthalpy_birthdeath_varlabel, Ghost::None, 0 );
      ...
      tsk->requires( Task::WhichDW::NewDW, particle_enthalpy_varlabel, Ghost::None, 0 );
      ...
      tsk->requires( Task::WhichDW::NewDW, particle_temperature_varlabel, Ghost::None, 0 );
    }
