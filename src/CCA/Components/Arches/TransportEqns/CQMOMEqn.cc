...
CQMOMEqn::sched_initializeVariables( const LevelP& level, SchedulerP& sched )
{
  ...
  //Old
  tsk->requires(Task::WhichDW::OldDW, d_transportVarLabel, gn, 0);
...
CQMOMEqn::sched_buildTransportEqn( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  ...
  tsk->requires(Task::WhichDW::NewDW, d_oldtransportVarLabel, Ghost::AroundCells, 2);
  tsk->modifies(d_FdiffLabel);
  tsk->modifies(d_RHSLabel);

  //-----OLD-----
  tsk->requires(Task::WhichDW::OldDW, d_fieldLabels->d_areaFractionLabel, Ghost::AroundCells, 2);
  tsk->requires(Task::WhichDW::OldDW, d_transportVarLabel, Ghost::AroundCells, 2);
  tsk->requires(Task::WhichDW::OldDW, d_fieldLabels->d_viscosityCTSLabel, Ghost::AroundCells, 1);
  tsk->requires(Task::WhichDW::OldDW, d_fieldLabels->d_cellTypeLabel, Ghost::AroundCells, 1);

  if (d_addSources) {
    tsk->requires(Task::WhichDW::NewDW, d_sourceLabel, Ghost::None, 0);
  }

  if (d_usePartVel) {
    tsk->requires( Task::WhichDW::NewDW, d_FconvLabel, Ghost::None, 0);
  } else {
    if (timeSubStep == 0 ) {
      tsk->requires(Task::WhichDW::OldDW, d_fieldLabels->d_uVelocitySPBCLabel, Ghost::AroundCells, 1);
#ifdef YDIM
      tsk->requires(Task::WhichDW::OldDW, d_fieldLabels->d_vVelocitySPBCLabel, Ghost::AroundCells, 1);
#endif
#ifdef ZDIM
      tsk->requires(Task::WhichDW::OldDW, d_fieldLabels->d_wVelocitySPBCLabel, Ghost::AroundCells, 1);
#endif
    } else {
      tsk->requires(Task::WhichDW::NewDW, d_fieldLabels->d_uVelocitySPBCLabel, Ghost::AroundCells, 1);
#ifdef YDIM
      tsk->requires(Task::WhichDW::NewDW, d_fieldLabels->d_vVelocitySPBCLabel, Ghost::AroundCells, 1);
#endif
#ifdef ZDIM
      tsk->requires(Task::WhichDW::NewDW, d_fieldLabels->d_wVelocitySPBCLabel, Ghost::AroundCells, 1);
#endif
    }
...
CQMOMEqn::sched_solveTransportEqn( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  ...
  tsk->requires(Task::WhichDW::NewDW, d_RHSLabel, Ghost::None, 0);

  //Old
  tsk->requires(Task::WhichDW::OldDW, d_transportVarLabel, Ghost::None, 0);
  tsk->requires(Task::WhichDW::OldDW, d_fieldLabels->d_delTLabel, Ghost::None, 0 );
  tsk->requires(Task::WhichDW::OldDW, d_fieldLabels->d_volFractionLabel, Ghost::None, 0 );
...
CQMOMEqn::sched_buildXConvection( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  ...
  for (ArchesLabel::WeightMap::iterator iW = d_fieldLabels->CQMOMWeights.begin(); iW != d_fieldLabels->CQMOMWeights.end(); ++iW) {
    ...
    tsk->requires( Task::WhichDW::NewDW, tempLabel, Ghost::AroundCells, 2 );
  }
...
CQMOMEqn::sched_buildSplitRHS( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  ...
  tsk->modifies(d_FconvLabel);
  tsk->modifies(d_RHSLabel);
  tsk->requires(Task::WhichDW::NewDW, d_tempLabel, Ghost::None, 0);

  tsk->requires(Task::WhichDW::NewDW, d_FconvXLabel, Ghost::None, 0);
  tsk->requires(Task::WhichDW::NewDW, d_FconvYLabel, Ghost::None, 0);
  tsk->requires(Task::WhichDW::NewDW, d_FconvZLabel, Ghost::None, 0);
  ...
  if (timeSubStep == 0) {
    tsk->requires(Task::WhichDW::OldDW, d_sourceLabel, Ghost::None, 0);
  } else {
    tsk->requires(Task::WhichDW::NewDW, d_sourceLabel, Ghost::None, 0);
  }
  ...
  for (ArchesLabel::WeightMap::iterator iW = d_fieldLabels->CQMOMWeights.begin(); iW != d_fieldLabels->CQMOMWeights.end(); ++iW) {
    const VarLabel* tempLabel = iW->second;
    tsk->requires( Task::WhichDW::OldDW, tempLabel, Ghost::AroundCells, 1 );
  }
  for (ArchesLabel::AbscissaMap::iterator iA = d_fieldLabels->CQMOMAbscissas.begin(); iA != d_fieldLabels->CQMOMAbscissas.end(); ++iA) {
    const VarLabel* tempLabel = iA->second;
    tsk->requires( Task::WhichDW::OldDW, tempLabel, Ghost::AroundCells, 1 );
