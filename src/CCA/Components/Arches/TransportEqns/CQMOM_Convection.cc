...
CQMOM_Convection::sched_solveCQMOMConvection( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  ...
  tsk->requires(Task::WhichDW::OldDW, d_fieldLabels->d_volFractionLabel, Ghost::AroundCells, 1);
  ...
  for (ArchesLabel::WeightMap::iterator iW = d_fieldLabels->CQMOMWeights.begin(); iW != d_fieldLabels->CQMOMWeights.end(); ++iW) {
    ...
    if (timeSubStep == 0 ) {
      tsk->requires( Task::WhichDW::OldDW, tempLabel, Ghost::AroundCells, 2 );
    } else {
  ...
  if ( d_deposition ) {
    for ( int i = 0; i < nNodes; i++ ) {
      tsk->requires( Task::WhichDW::NewDW, fStickLabels[i], Ghost::None, 0 );
    }
  }

  tsk->requires( Task::WhichDW::NewDW, d_wallIntegerLabel, Ghost::AroundCells, 1);
