...
void ScalarVarianceScaleSim::sched_computeProp( const LevelP& level, SchedulerP& sched, int time_substep )
{
  ...
  if ( time_substep == 0 ){ 
    ...
    tsk->requires( Task::WhichDW::OldDW, _mf_label, Ghost::AroundCells, 1 ); 
    tsk->requires( Task::WhichDW::OldDW, _density_label, Ghost::AroundCells, 1 ); 
    tsk->requires( Task::WhichDW::OldDW, _vol_frac_label, Ghost::None, 0 ); 

  } else { 

    tsk->requires( Task::WhichDW::NewDW, _mf_label, Ghost::AroundCells, 1 ); 
    tsk->requires( Task::WhichDW::NewDW, _density_label, Ghost::AroundCells, 1 ); 
    tsk->requires( Task::WhichDW::NewDW, _vol_frac_label, Ghost::None, 0 ); 

  } 

  tsk->requires( Task::WhichDW::NewDW, _filter_vol_label, Ghost::None, 0); 
  tsk->requires( Task::WhichDW::NewDW, _celltype_label,     Ghost::AroundCells, 1); 
