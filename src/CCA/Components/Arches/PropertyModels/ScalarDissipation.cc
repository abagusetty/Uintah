...
//Method: Constructor
//---------------------------------------------------------------------------
ScalarDissipation::ScalarDissipation( std::string prop_name, MaterialManagerP& materialManager ) : PropertyModelBase( prop_name, materialManager )
{
...
void ScalarDissipation::sched_computeProp( const LevelP& level, SchedulerP& sched, int time_substep )
{
  ...
  if ( time_substep == 0 ){ 
    ...
    tsk->requires( Task::WhichDW::OldDW, _gradmf2_label, Ghost::None, 0 ); 
  } else { 
    
    tsk->requires( Task::WhichDW::NewDW, _gradmf2_label, Ghost::None, 0 ); 
  } 
