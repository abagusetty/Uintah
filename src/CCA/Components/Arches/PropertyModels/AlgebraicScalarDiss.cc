...
//Method: Constructor
//---------------------------------------------------------------------------
AlgebraicScalarDiss::AlgebraicScalarDiss( std::string prop_name, MaterialManagerP& materialManager ) : PropertyModelBase( prop_name, materialManager )
{
...
void AlgebraicScalarDiss::sched_computeProp( const LevelP& level, SchedulerP& sched, int time_substep )
{
  ...
  if ( time_substep == 0 ){ 
    ...
    tsk->requires( Task::WhichDW::OldDW, _mu_t_label, Ghost::None, 0 ); 
    ...
    tsk->requires( Task::WhichDW::NewDW, _mu_t_label, Ghost::None, 0 ); 
