...
void NormScalarVariance::sched_computeProp( const LevelP& level, SchedulerP& sched, int time_substep )
{
  ...
  if ( time_substep == 0 ){ 
    ...
    tsk->requires( Task::WhichDW::OldDW, _mf_m2_label, Ghost::None, 0 ); 
    tsk->requires( Task::WhichDW::OldDW, _vf_label, Ghost::AroundCells, 1); 
  } else { 
    tsk->requires( Task::WhichDW::NewDW, _mf_label, Ghost::AroundCells, 1 ); 
    tsk->requires( Task::WhichDW::NewDW, _mf_m2_label, Ghost::None, 0 ); 
    tsk->requires( Task::WhichDW::NewDW, _vf_label, Ghost::AroundCells, 1); 
  } 
